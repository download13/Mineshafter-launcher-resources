...
public class Http {
	...
	public static String performPost(URL url, String parameters, Proxy proxy, String contentType, boolean returnErrorPage) throws IOException {
		...
		//System.out.println("Http.java - Post: "+url.toString());
		...
		try {
			...
			if (returnErrorPage) {
				...
				if (stream != null) reader = new BufferedReader(new InputStreamReader(stream));
				else throw e;
			} else {
				throw e;
			}
	...
	public static String performGet(URL url, Proxy proxy) throws IOException {
		...
		//System.out.println("Http.java - Get: "+url.toString());
	...
	public static URL concatenateURL(URL url, String args) throws MalformedURLException {
		if ((url.getQuery() != null) && (url.getQuery().length() > 0)) { return new URL(url.getProtocol(), url.getHost(), url.getFile() + "?" + args); }
